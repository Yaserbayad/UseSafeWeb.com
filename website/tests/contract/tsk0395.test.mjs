...
for (const locale of locales) {
  test(
    `TSK-0395 ${locale} landing proposition is first-phone-led rather than DNS-led`,
    () => {
      ...
      assert.doesNotMatch(
        content.home.title,
        /dns/i,
        'landing headline must not make DNS the product proposition',
      );
      assert.doesNotMatch(
        content.home.summary,
        /^dns/i,
        'landing summary must lead with the user outcome, not DNS',
      );
...
test(
  'TSK-0395 landing page renders canonical locale content and routes the primary CTA to accountless setup',
  () => {
    const page = read('src/app/[locale]/page.tsx');

    assert.match(
      page,
      /publicMetadata\(locale, '', content\.home\.title, content\.home\.summary\)/,
    );
...
test(
  'TSK-0395 public landing retains semantic heading, keyboard-native links, and shared responsive design-system classes',
  () => {
