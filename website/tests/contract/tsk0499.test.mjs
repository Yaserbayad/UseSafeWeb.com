...
test('preserves six protection states and never converts parent confirmation into positive technical verification', async () => {
  ...
  const stateEvent = {
    ...
    journey_session_id: UUID_B,
    layer_id: 'internet',
    state: 'configured_parent_confirmed',
    reason_code: 'parent_confirmed',
    copy_version: '1.0.0',
  };
  ...
  const verifierEvent = {
    ...
    journey_session_id: UUID_B,
    layer_id: 'internet',
    verifier_id: 'dns-path',
    verifier_version: '1.0.0',
    result: 'positive',
    reason_code: 'parent_confirmed',
    duration_bucket_ms: 'lt_1000',
  };
...
test('enforces non-sliding 24h accountless retention, 30d synthetic retention, conservative 13m cost retention, dedupe and deletion', async () => {
  ...
  assert.ok(
    api.MEASUREMENT_MAX_AGE_MS <= 13 * 30 * 24 * 60 * 60 * 1000,
    'fixed-duration measurement retention must not exceed the conservative 13-month bound',
  );
  ...
  const cost = {
    ...
    provider_or_source_class: 'azure',
    currency: 'EUR',
    amount_minor_units: 1234,
    source_reference: 'invoice-2026-09',
...
test('client timestamps cannot extend raw retention and deletion cannot reset an accountless session hard-expiry', async () => {
  ...
  assert.ok(
    future.expiresAt <= t0 + api.ACCOUNTLESS_RAW_MAX_AGE_MS,
    'client-controlled occurred_at must not extend retention beyond receipt time + TTL',
  );
  ...
  assert.equal(
    later.expiresAt,
    original.expiresAt,
    'deleting the last raw record must not reset the originating session TTL',
  );
});
