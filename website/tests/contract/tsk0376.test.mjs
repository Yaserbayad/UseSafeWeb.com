...
test('canonical happy path exposes Phone -> Internet -> Services without a parallel persisted stage model', async () => {
  ...
  assert.equal(state.verification.state, 'protected/verified');
  assert.equal(state.verification.reasonCode, 'TECH_VERIFIED');
});
...
test('parent confirmation and measured verification remain distinct and an evidence-free UX event cannot manufacture verified/protected state', async () => {
  ...
  assert.equal(confirmed.verification.state, 'configured/parent-confirmed');
  assert.equal(confirmed.verification.reasonCode, 'CONFIG_CONFIRMED_NO_TECH_VERIFY');
  ...
  assert.equal(measured.verification.state, 'protected/verified');
  assert.equal(measured.verification.reasonCode, 'TECH_VERIFIED');
});
...
test('negative/uncertain verification cannot enter Services and recovery retry is bounded and deterministic', async () => {
  ...
  assert.equal(api.coreJourneyStage(state), 'internet');
  assert.equal(state.verification.state, 'action-needed');
  ...
  for (let attempt = 1; attempt <= api.CORE_MAX_VERIFICATION_RETRIES; attempt += 1) {
    ...
    assert.equal(first.verification, null, 'retry must clear stale/elevated verification state');
...
test('Journey-0 state is accountless, session-scoped, capped at 24h, and malformed/expired state restarts safely', async () => {
  ...
  for (const forbidden of ['account', 'household', 'child', 'browsing', 'history', 'dnsquery', 'hostname', 'domain']) {
    assert.equal(serialized.includes(forbidden), false, `retained core state contains prohibited field ${forbidden}`);
  }
});
