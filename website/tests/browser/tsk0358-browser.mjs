...
await record('accountless core fails closed at uncertain verification and truthful Protection Map remains available only from valid protection state', async () => {
  ...
  await page.waitForFunction(() => document.querySelector('[data-verification-outcome]')?.getAttribute('data-verification-outcome') === 'uncertain');
  assert.equal(await verifyPanel.getAttribute('data-protection-state'), 'uncertain/error');
  assert.equal(await page.locator('[data-core-view-protection]').count(), 0, 'uncertain verification must not enter Services');
  ...
  assert.equal('email' in state || 'accountId' in state || 'history' in state || 'query' in state || 'domain' in state || 'verification' in state, false);
