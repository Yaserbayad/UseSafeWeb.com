...
- ordinary host workflows use trusted `main`, `contents: read`, `persist-credentials: false`, and serialized `usesafeweb-adguard-server` concurrency.

Manual per-command SSH execution is no longer the normal path.
...
- `TSK-0204` — persistent query/file logging disabled — evidence: `TSK_0204_QUERYLOG_PRIVACY_EVIDENCE_2026-08-27.md`, blob `79b0e5f4c42eadc8e7ecf7f7598a1b6ad1bcc785`.

### TSK-0204 accepted stable state

Mutation run `33122182026` / job `98691510253`: PASS.  
Independent audit run `33122229571` / job `98691673148`: PASS.

Direct target evidence:

- authenticated query-log config reports `enabled=false`;
- existing query-log history was cleared;
- mutation synthetic query `usesafeweb-log-test.invalid` was not retained;
- fresh-audit synthetic query `usesafeweb-fresh-audit.invalid` was not retained;
- query-log API item count remained `0`;
- no non-empty `querylog.json*` file exists beneath `/opt/AdGuardHome`;
- AdGuard and runner services remained active;
- no real user query/domain history was used as evidence.

ACC-0204 is fully satisfied.

### Selected next

`TSK-0205` — disable or exclude identifiable per-client statistics: **TODO / selected**. Acceptance requires no identifiable per-client statistics to be stored unless explicitly justified and both UI/storage to reflect the intended privacy state. Current configuration must be inspected through the versioned v0.107.79 statistics API/schema before mutation.

### Subsequent current-gate work

- `TSK-0206` — enable client-IP anonymisation: waits on TSK-0205 PASS.
- `TSK-0483` — resolver abuse/amplification controls: dependency-eligible after TSK-0203; remains ordered after immediate privacy controls and mandatory before public resolver activation.
- `TSK-0407` — configure Quad9 dns10 / ECS off: execute after immediate privacy controls in current security/privacy ordering, subject to verified predecessor state.
...
Inspect the official AdGuard Home v0.107.79 statistics-control API/schema, then execute and independently audit the smallest configuration change satisfying TSK-0205 without weakening unrelated statistics or resolver settings. Persist PASS evidence and reread runtime before selecting TSK-0206.
