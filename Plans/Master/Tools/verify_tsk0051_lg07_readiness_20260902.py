...
def main():
    ...
    for x in ['architecture and implementation plan','account/session/data/vendor/privacy/security model','auth/authz/csrf/idor/deletion/recovery','typed adguard integration','measurement','test/recovery','budget','delivery plan','residual risks','project governance / auto_allowed','unlocks l6 build only on evidence-complete pass']:
        req(x in lg07,f'missing LG-07 gate clause: {x}')
    ...
    art=ART.read_text(encoding='utf-8'); low=art.lower()
    req('decision candidate' in low and '**pass**' in low[:700],'LG-07 decision candidate is not PASS')
    for x in ['architecture and implementation plan','cp-lg07-01','planned initial integrated implementation checkpoint','accountless core remains complete without login','optional parent-account boundary is implemented','csrf/idor/cross-parent isolation','query/statistics-history suppression','mandatory login, browsing/query/activity history, child accounts and unrestricted customer dns administration remain excluded','zero incremental new development spend','contingency: **0**','no unresolved high/critical architecture/control-plan gap','tsk-0050 must then persist the approved baseline/readiness decision','does not claim completed l6 implementation']:
        req(x in low,f'missing LG-07 decision boundary: {x}')
    backlog=BACKLOG.read_text(encoding='utf-8'); blow=backlog.lower()
    for x in ['current non-pass l6 tasks represented: **76**','dependency-ordered execution slices: **55**','preserve the complete **accountless core**','optional version-1 scope may include google sign-in/server session','slice 30','implement firebase google sign-in and privacy-minimal server session lifecycle','slice 38','implement dashboard shell, empty state and parent-owned device cards','coverage checkpoints required before lg-07']:
        req(x in blow,f'backlog/checkpoint evidence missing: {x}')
    req('no unresolved high/critical architecture/control-plan gap within this component' in state.lower(),'TSK-0049 no-critical-design-gap evidence missing')
    print('PASS: TSK-0051/LG-07 current contracts, 20 evidence anchors, residual L5 frontier, integrated checkpoint, risk/cost/legal fences and L6 non-inference verified.')
