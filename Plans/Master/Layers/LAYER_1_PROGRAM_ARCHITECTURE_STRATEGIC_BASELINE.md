...
| UK-targeted first; i18n/RTL-ready | Premature full localization or un-gated official expansion |
...
The governing optimization order is: **correct product -> exceptional UX/UI -> customer value -> reliability/trust -> quality -> simplicity -> AI autonomy -> execution efficiency -> operational efficiency -> cost efficiency**. No package may reverse this order merely to appear faster or cheaper.
...
| Architecture readiness | English externalized content; Turkish/Arabic-capable translation files, locale metadata, content applicability, and RTL layout support. |
| Progressive localization | Do not require full Turkish/Arabic product/content/support/legal/channel work before evidence and owner activation. |
...
- Core safety outcome remains free; no card/trial/payment before value; supporter hypothesis may be tested after value without changing protection.
...
Initial architecture deliberately accepts approximately **30 minutes of service downtime/recovery** for server failure or planned rebuild. Near-zero-downtime HA is not a current requirement. One production-grade Bash deployment/recovery path must start from a supported fresh Ubuntu 24.04 LTS server and restore a verified DNS service end-to-end, including host baseline, packages, AdGuard, approved configuration, firewall/network, endpoint, TLS, filters, security/privacy settings, service startup, automated verification, and health. Actual restoration time, not script runtime alone, is the acceptance measure.
...
| PKG-07 | Web Experience & Application Engineering | Direct customer value after validation | Software / Frontend / Backend Engineering | A4 in tested non-production delivery; A2 for production deployment/data changes; A1 for irreversible external actions |
...
| PKG-09 | Cloud Infrastructure & Platform Engineering | Mandatory enabling control | Cloud / Platform Engineering | A4 for IaC and rehearsed recovery in non-production; A2 for production provision/change/destroy |
...
| PKG-13 | Service Operations, Reliability & Technical Support | Mandatory enabling control | SRE / Operations | A4 for monitoring, diagnosis, runbook execution, safe recovery; A2 for production remediation; A1 for service-disable/incident authority where material |
